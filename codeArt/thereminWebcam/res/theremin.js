class WebcamTheremin {
  ...
  init() {
    ...
    // Wait for user gesture to start theremin
  ...
  setupEventListeners() {
    ...
    oscillatorSelector.addEventListener("change", (e) =>
      this.changeOscillatorType(e.target.value),
    );
  ...
  setupBeginButton() {
    const beginBtn = document.getElementById("beginThereminBtn");
    const thereminInterface = document.getElementById("theremimInterface");
    if (beginBtn && thereminInterface) {
      beginBtn.addEventListener("click", async () => {
        ...
        thereminInterface.style.display = "";
      });
    }
  }
  ...
  async autoStartTheremin() {
    // Removed auto-start for mobile compatibility
    // Now started by user gesture (button)
  }
  ...
  processThresholdImage() {
    ...
    this.thresholdCtx.clearRect(
      0,
      0,
      this.thresholdCanvas.width,
      this.thresholdCanvas.height,
    );
    this.thresholdCtx.drawImage(
      tempCanvas,
      0,
      0,
      this.thresholdCanvas.width,
  ...
  updateFrequency(brightnessPercent) {
    ...
    const noteIndex = Math.floor(
      (brightnessPercent / 100) * (this.musicalNotes.length - 1),
    );
    const clampedIndex = Math.max(
      0,
      Math.min(this.musicalNotes.length - 1, noteIndex),
    );
    ...
    if (
      noteDuration > this.fadeStartDuration &&
      ...
      const fadeProgress =
        (noteDuration - this.fadeStartDuration) /
        (this.maxNoteDuration - this.fadeStartDuration);
  ...
  updateChart() {
    ...
    if (this.brightnessHistory.length > 0) {
      ...
      const y =
        chartHeight - (this.brightnessHistory[lastIndex] / 100) * chartHeight;
